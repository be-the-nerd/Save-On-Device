package com.divyansh.saveondevice
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (intent?.action) {
            ...
            Intent.ACTION_SEND_MULTIPLE -> {
                inputUriList =
                    (intent.extras?.getParcelableArrayList<Uri>(Intent.EXTRA_STREAM) as ArrayList<Uri>).toMutableList()
                val uri = inputUriList.removeFirstOrNull()
    ...
    private fun callSaveFileResultLauncher(uri: Uri) {
        if(isSupportedMimeType(uri)) {
            ...
            Toast.makeText(applicationContext, "Unsupported Mimetype", Toast.LENGTH_SHORT).show()
        }
    }
